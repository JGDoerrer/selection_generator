...
The \emph{dual} of a poset $R$ is obtained by reversing the direction of all edges, \ie $R^{-1} = \set{(v,u)}{(u,v) \in R}$.
...
Since in this paper we only look at posets in the context of the selection problem, we extend the definition of a poset, to include a rank $i$.
That is, a poset is a tupel $(n, i, R)$ where $n = |\Omega_R|$.
The notion of isomorphism extends naturally to this extended definition of a poset.
For the dual we have $(n, i, R)^{-1} = (n, n-i+1, R^{-1})$.
We say that two poset $R$ and $Q$ are \emph{congruent} if $P$ is isomorphic to $Q$ or $Q^{-1}$.
A poset is \emph{reduced} if for each element there are at most $i-1$ smaller elements and at most $n - i$ larger elements.
...
The leaf nodes are labeled with posets that have a unique element $a$, such that, there are $i-1$ elements smaller than $a$, and $n- i$ elements larger than $a$.
That is, $a$ ist the $i$-th smallest element.
