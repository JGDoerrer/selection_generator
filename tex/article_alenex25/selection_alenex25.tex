...
The \emph{dual} of a poset $R$ is obtained by reversing the direction of all edges, \ie $\dual{R} = \set{(v,u)}{(u,v) \in R}$.
...
Note that the bound in \Cref{lemma:previous_next_poset}\todo{use cleveref everywhere} is tight, as can be seen by $V_1(n) = n-1$\todo{cite, or mention in related work}.
