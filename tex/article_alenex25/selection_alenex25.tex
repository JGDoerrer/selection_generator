...
\usepackage[letterpaper]{geometry} % todo: is this out-commented right?
...
Looking at the special case of selecting the median $i = \nicefrac{n}{2}$, the best-known algorithm requires $2.95n$ comparisons~\cite{dor1999selecting}. % todo: best known ask alex
...
The theoretical upper bounds for $V_7(16)$ and $V_8(16)$ have already been determined by Gasarch~\cite{Gasarch1996}.
...
\begin{table}[!t]
  ...
  \caption{Execution times of different search methods.
    ...
    Note that the value for $n = 15$ and $i = 8$ for the forward search is from an older version and should run slightly faster in the current version.
  }
...
An alternative approach to the bidirectional search would be to first run the backward search, but restrict it to problems with specific properties such as a large number of compatible solutions, e.g.\ problems $(P, i)$ in $A_k$ with $\mathcal{C}(P, i) \ge \alpha \cdot 2^k$ for some $\alpha \in [0, 1]$.
...
If true, it would imply a $2.5n + \smallO(n)$ algorithm for computing the median. % todo: cite
