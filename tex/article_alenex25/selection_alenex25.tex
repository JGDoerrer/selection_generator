...
\begin{abstract} \small\baselineskip=9pt This research project aims to find worst case optimal comparison algorithms for selecting the i-th smallest of n elements of
...
The  problem of selecting the $i$-th smallest element in a
...
measuring the cost as the number of comparisons made.
...
A partially ordered set, short \emph{poset}, is a set $\Omega$ with a partial order $R \subseteq \Omega \times \Omega$.
By a slight abuse of notation, we denote the poset by $R$ as well.
Where necessary, we write $\Omega_R$ for the underlying set.
...
The \emph{dual} of a poset is obtained by reversing the direction of all edges, \ie $\set{(v,u)}{(u,v) \in R}$.
...
An instance of the selection problem is a tupel $(n, i, R)$ denoting the problem of finding the $i$-th smallest of the $n$ elements in $\Omega_R$ where we already know the relations $R$.
That means we assume $|\Omega_R| = n$.
\todo[inline]{Do we call the tupel a poset too, or should we have a different name?}
\todo[inline]{I changed $i$-th largest to $i$-th smallest.}
\todo[inline]{Can we have $i$ start at 1 in the paper?}

\todo[inline]{The following definitions could be text.}
...
\begin{definition}[Reduced] \label{definition:reduced_poset}
  ...
  Hence, all elements can still be the $i$-th smallest.
\end{definition}

\subsection{Selection Algorithms.}
A selection algorithm is a binary decision tree.
Each node is labeled with a poset.
The root node is labeled with $(n, i, \emptyset)$.
The leaf nodes are labeled with posets that have a unique element $u$, such that, there are $i-1$ elements smaller than $u$, and $n- i$ elements larger than $u$.
That is, $u$ ist the $i$0th smalles element.

The selection algorithm associates to each inner node $P$ a comparison $\{u,v\}$, meaning that the algorithm compares $u$ with $v$ as its next step.
The two children $\pchild{P}{u}{v}$ and $\pchild{P}{v}{u}$ correspond to the two possible outcomes of the comparison $u \preceq v$ and $u \succeq v$.
...
\todo[inline]{This def could be text. How does it compare to the notation of knuth?} % V_i(n); V(P) Notation von Knuth
...
\todo[inline]{The following lemma could be text.}
...
\begin{definition}[Compatible Solution] % hier p+uv
...
\todo[inline]{The following could be an observation, then skip the proof.}
...
\begin{proof}
  \todo[inline]{Proof can be much shorter if we do away with the induction, and argue with the height of a binary tree.}
...
\begin{figure}[!b]
  ...
  \caption{Finding the median $i = 2$ of $n = 5$ values using six comparisons.\todo[inline]{Can we make this smaller/ same style as the other figures?}}
...
In this section we describe our three main approaches -- forward search, backward search and bidirectional search.
...
\todo[inline]{Remove bidirectional search?}


\todo[inline]{I put this here for now. Maybe we can have a subsection Datastructures and Isomorphism testing?}
...
\todo[inline]{Similar to Oksanen, we use a minimax algorithm to computet the cose $V(n, i, P)$...}
The forward search starts with an unordered poset $P=(n,i,\emptyset)$, and recursively determines solvability of a given poset within some bound by exhaustively searching the results of all possible comparisons to be made unless it is already solved or we determine that it cannot be solved within the alloted number of comparisons by some solvability-check.

Between the two possible outcomes of a comparison we assume the worse, but since an algorithm is free to choose which elements to compare we are looking for the most useful comparison, the one that yields result posets cheapest to solve themselves\todo{?}, still in terms of worst case outcome.
...
To limit memory cost and allow further pruning we traverse the search tree depth first, reducing the maximum number of comparisons alloted\todo{?} to child posets to one less than the current best result found.
...
\subsubsection{Pruning Decisions} % hier einfach schreiben: wir machen alpha-beta pruning
...
It is optimal because of an inductive argument over the number of relative orders\todo{?} determined.
An already solved poset obviously takes no further comparisons to solve.
Then, given an unsolved poset with all elements already too large or small excluded\todo{reduced} we must make at least one more comparison to
make progress.
Since all posets resulting from these comparisons must have more elements ordered\todo{?} than the current one has we can assume as per induction that we know their optimal cost.
...
\todo[inline]{This could go to the datastructures and canonification section}
...
This means that the following heuristics must only provably be not too pessimistic.\todo{?} % das sollte hoch
...
% If the new poset is found to be solvable in the given number of comparisons, we estimate that the original poset is also solvable.\todo{?}
...
\todo[inline]{We could remove this.}
...
The backward search starts with the solved poset and iteratively removes comparisons until the unordered poset is found.
...
\todo[inline]{Swap definition with lemma}
\begin{definition}[Predecessor] \label{definition:predecessor_calculation}
  ...
  \begin{itemize}
    ...
    \item There exists a comparison\todo{Hasse arc} $(i, j)$ whose adding in $Q$ and subsequent normalization results in the poset $P$ and adding the reverse comparison $(j, i)$ and subsequent normalization results in an already found poset
  \end{itemize}
\end{definition}
...
\todo[inline]{I would probably put the following explanation of the "table optimization" in a separate Implementation details/ optimizations section.}
...
\todo[inline]{Have a short itemize, describing each step in one sentence, before going into the details for each step.}
\begin{itemize}
  \item[1.]
  ...
  \item[2.]
    ...
    \todo[inline]{We need the other lemma here too.}
  ...
  \item[3.]
    ...
    When inserting a new element, it is important to note that it may no longer be the $i$-th smallest but rather the $(i + 1)$-th smallest element being searched, similar to the second step, and thus cannot be directly reduced.\todo{More precise?}
  ...
  \item[4.]
    \todo[inline]{This isn't really the last, step, is it? This is an optimization that we apply at every step.}
...
Finally, the posets elements are arranged canonically. % TODO: nauty in textttt
The canonic labelling of the elements is provided by nauty\todo{Wie schreiben wir nauty}, which is a program in \texttt{C} for the calculation of graph automorphism groups \cite[Practical Graph Isomorphism]{MCKAY201494}.
...
\begin{figure}[!b]
  ...
  \caption{Problematic case where nauty cannot distinguish between the two posets, despite them being each other's duals. However, according to Lemma~\ref{lemma:dual_poset_allowed} they can be transformed into each other.\todo[inline]{Nauty cannot distinguish?}} % KEIN FEHLER, da gerichteter Graph
