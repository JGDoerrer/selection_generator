...
This observation leads us to the notion of compatible solutions, which, similar to linear extensions, is one such partition compatible with the current relation.
...
Then, the new poset is then searched for a solution using the forward search described above without reducing the number of allowed comparisons.
...
For $n=14,i=7$ this resulted in a time of 1h~21m with $147\cdot10^6$ posets in cache, an improvement by factors $11$ and $6.3$ respectively. % hä? Satzbau
...
The latest version of our software can be found on \projectServer~(\url{\projectURL}).
...
assume the two searches meet at the point where $11$ comparisons have been performed.
Then at the meeting point, both have alread covered $99\%$ of their search space.
...
\begin{figure}[h]
  ...
  \caption*{Cost distribution for all instances solved in the forward search for $n\leq 14$, excluding $i\leq 2$, purple are low $i$, yellow high. $\log_2$ in red.} % Satzbau
\end{figure}
