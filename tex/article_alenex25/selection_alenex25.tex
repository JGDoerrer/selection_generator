...
\author{Josua Dörrer, Konrad Gendle, Johanna Hofmann \\ Julius von Smercek, Andreas Steding, Florian Stober} % todo-final-version
...
Note that the bound in \Cref{lemma:previous_next_poset} is tight, as can be seen from $V_1(n) = n - 1$.
...
% todo-final-version @Florian: Please read the lemma & proof again.
\begin{proof} \label{proof:predecessor_calculation}
  Since $(Q, j)$ is a predecessor of $(P, i)$, $(a, b)$ exists with $V_j(\pchild{Q}{b}{a}) \leq V_i(P)$.
  Obviously, $V_j(Q) \leq V_j(\pchild{Q}{b}{a}) + 1$ applies, since adding a comparison increases $V_j(Q)$ by a maximum of one.
  Therefore $V_j(Q) \leq V_i(P) + 1$ follows.
\end{proof}
...
\end{enumerate}
We describe the individual steps below. %todo: mention only store new predecessors in $A_{k+1}$
...
Otherwise, as proven in \Cref{lemma:remove_only_last_element_edge}, if one of $a$ or $b$ is in $P$, the predecessor is enumerated in this second step.
...
In \Cref{fig:backward_canonify_problematic}, the posets appear different in the Hasse diagram despite being each other's duals.
...
Implementing this canonification preprocessing significantly reduces the number of cases requiring \texttt{nauty}, as illustrated in \Cref{table:nauty-ratio}.
...
%todo: Where do the upper bounds for the ranges come from? -> Gasarch & Knuth
\begin{table}[!t]
  \renewcommand{\arraystretch}{1.1}
...
\Cref{table:num-comparisons} shows the values $V_i(n)$, representing the minimum number of comparisons needed to solve the selection problem for $n \leq 16$.
...
%todo: 15, 8 is an old value from the forward search (without compatible posets)
\begin{table}[!t]
  \renewcommand{\arraystretch}{1.1}
...
\Cref{table:search_algorithms} compares the execution times of different algorithms to find optimal selection algorithms.
All experiments were conducted on a machine with two Intel Xeon CPUs, each equipped with $12$ cores ($24$ threads), and a total of $768$ GB of RAM. % todo: Hardware is also in the appendix -> twice
