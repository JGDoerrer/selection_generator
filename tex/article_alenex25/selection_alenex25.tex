...
\begin{abstract} \small\baselineskip=9pt
  This research project aims to find worst case optimal comparison algorithms for selecting the $i$-th smallest of $n$ elements of a set for $n$ up to $16$ with computer search.
...
On his website Kenneth Oksanen~\cite{Oksanen} published a computer search algorithm improving the previously known lower bounds found by Gasarch et.\ al\@.~\cite{Gasarch1996}.
...
Notation: $\less{P}{v} = {v\downarrow_P} = D_P(v)$ and $\greater{P}{u} = u \uparrow_P = U_P(u)$.
}


\subsection{The Selection Problem.}
The selection problem is, given a poset $R$ and an integer $i$, to determine the $i$-th smallest of the $n$ elements in $\Omega_R$ where we already know the relation $R$.
We denote an instance of the \emph{selection problem}, or problem for short, by $(R, i)$\todo{alternative: instance}.
...
\begin{proof}
  ...
  In the input of the algorithm we insert a new element $\omega$ that is larger than any other element.
...
Note that the bound in \Cref{lemma:previous_next_poset} is tight, as can be seen by $V_1(n) = n - 1$\todo{cite, or mention in related work}.
...
\begin{definition}[Canonified]
  ...
  \begin{itemize}
    ...
    \item
          ...
          This is also described in more detail in \Cref{sec:backward:normal_form}
  \end{itemize}
...
\begin{enumerate}
  ...
  \item[2.]
    Use \texttt{current\_best}.
    This variable facilitates cutting off branches according to the idea of alpha-beta pruning.
\end{enumerate}
...
Then, given an unsolved poset with all elements already too large or small excluded\todo{reduced} we must make at least one more comparison to make progress.
...
\begin{figure}[!b]
  \centering
  \includegraphics[width=\columnwidth]{figures/backward-search-tree.png} \todo[inline]{this graphic should be adjusted to the shifted i}
...
\begin{table}[!t]
  ...
  \end{tabular}
\end{table}
\todo[inline]{note that these times are from an older version}
...
The values we calculated as the minimum number of comparisons needed to solve the selection problem for $n \leq 16$ are shown in \Cref{table:num-comparisons}.
