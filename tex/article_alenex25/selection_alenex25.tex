...
This presents a significant gap to the best known lower bound, which is $\left (1 + H(\nicefrac{i}{n}) \right ) \cdot n + \Omega(\sqrt n)$ where $H(x) = x \cdot \log_2 \frac{1}{x} + (1 - x) \log_2 \frac{1}{1 - x}$~\cite{bent1985finding}.
...
\todo[inline]{I put most of this section in the introduction, so maybe this can go?}
...
\todo[inline]{Remark: It appears Oksanen did not know Lemma 2.1.}
...
\subsection{Example.}
\todo[inline]{Example weg oder in den Anhang, um auf 10 Seiten zu kommen.}
\begin{figure}[!b]
  \centering
...
\begin{algorithm}[t]
  ...
  \caption{An algorithm for computing the number of compatible solutions for a given poset.\todo[inline]{Use $\greater{P}{u}$.}}
...
With that restriction the starting point of the backward search is $(\emptyset_1, 1)$.\todo{$\emptyset_n$ -> $E_n$ + definieren}
