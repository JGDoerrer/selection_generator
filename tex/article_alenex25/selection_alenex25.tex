...
\todo[inline]{This def could be text. How does it compare to the notation of knuth?}
...
This means that the following heuristics must only provably be not too pessimistic.\todo{?}
...
The backward search starts with the solved poset and iteratively removes comparisons until the searched\todo{?} poset is found.
...
For this purpose, the poset is represented as a directed graph with respect to its Hasse diagram.\todo{?}
...
\begin{figure}[!b]
  ...
  \caption{Problematic case where Nauty cannot distinguish between the two posets, despite them being each other's duals. However, according to Lemma~\ref{lemma:dual_poset_allowed} they can be transformed into each other.\todo[inline]{Nauty cannot distinguish?}}
...
If all hash values are unique, the graph finally attains\todo{?} its unique normal form.
For two nodes with identical attributes\todo{?}, it is attempted to swap them.
