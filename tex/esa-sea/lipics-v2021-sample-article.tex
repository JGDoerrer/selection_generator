...
The fundamental approach shared by both search algorithms for 
detecting redundant Posets is to reorder the elements into 
a specific form that then can be tested for equality. The 
poset is reduced and converted into its dual if $i \leq \frac{n+1}{2}$.
If $i = \frac{n+1}{2}$, one of the two is chosen deterministically.
We then order the remaining elements based on their discovered relations 
as well as a hash-like value based on each element's neighborhood topology. We
found a depth of three for the considered neighborhood sufficient in almost all
cases.
...
Preliminary tests showed that the performance gained by
using this as is, where some isomorphic problems still may have different
representatives, outweighs the cost induced by the larger search space. 
\texttt{nauty} is never used.

\subsubsection{Backward Search} \label{sec:backward:normal_form}
