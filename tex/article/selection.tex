...
\begin{proof} \label{proof:inverse_poset}
  ...
  % "=>" Poset $p$ in $k$ Vergleichen lösbar => Inverse von Poset $p$ in $k$ Vergleichen lösbar
  %   Poset $p$ in $k$ Vergleichen lösbar
  %   => Dann gibt es einen Lösungalgorithmus dafür.
  %   => Wende diesen Lösungsalgorithmus an, wobei alle Vergleiche invertiert werden.
  %   => Dann bestimmt dieser das $(n - i - 1)$-te Elemenent.
  %   => das inverse Poset ist in $k$ Vergleichen lösbar. qed.
  % "<=" Inverse von Poset $p$ in $k$ Vergleichen lösbar => Poset $p$ in $k$ Vergleichen lösbar
  %   Inverse Poset $p$ in $k$ Vergleichen lösbar
  %   => Dann gibt es einen Lösungalgorithmus dafür.
  %   => Wende diesen Lösungsalgorithmus an, wobei alle Vergleiche invertiert werden.
  %   => Dann bestimmt dieser das $i$-te Elemenent.
  %   => das inverse vom inversen Poset ist in $k$ Vergleichen lösbar, d.h. $p$ qed.
\end{proof}
...
Nauty ist ein C-Programm zur Berechnung von Automorphismengruppen von Graphen und kann ein kanonisches Label für einen gegebenen Graphen erzeugen \cite{MCKAY201494}.
...
\begin{figure}[h!] % Das ist eigentlich unnötig -> bietet kein Mehrwert gegenüber 13,6
  \input{figures/tikz_backward_forward_12_5.tex}
  \centering
