...
\begin{itemize}
  ...
  \item $i\leq\frac{n+1}{2}$, using the dual.
...
While this canonical form has certain semantic properties, such as the elements being in a possible total order, the main incentive is unique representation with many aspects chosen for ease of computation instead. Some parts of this paper use a best effort approximation for this to increase speed, the results are then called \textbf{pseudo canonified}.
...
Solving a poset requires knowing the explicit elements making up these two sets. For suppose we have determined the element $e$ to be the solution, and an element $p$ remains unordered with $e$.
...
Because the less and greater sets need to be known, a solved poset has exactly one compatible solution and any solution compatible with a poset $P$ must be compatible with either $P+uv$ or $P+vu$ for any two unordered elements $u,v$, both if they are also unordered in the solution.
...
\begin{proof}
  If a poset is solvable within some number $k$ of comparisons there exists a binary decision tree of depth $k+1$. 
  Each leaf corresponds to exactly one compatible solution, so the tree must have at least as many 
  leafs as the poset at the root has compatible solutions. 
  With $\mathcal{C}(root)\leq2^{k+1}$ we get the proposed lower bound for $k$.
\end{proof}
