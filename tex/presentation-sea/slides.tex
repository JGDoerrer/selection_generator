...
\begin{frame}{\insertsection}
  \begin{itemize}
    ...
    \item<+-> Starts with the poset $(1, 0, \emptyset)$ % solved poset on the blackboard
    \item<+-> Iteratively computes all posets solvable in $k$ steps % k values on the blackboard
    \item<+-> Terminates when the poset $(n, i, \emptyset)$ is found % empty poset on the blackboard
  \end{itemize}
\end{frame}
...
\begin{frame}{Predecessor Computation}
  \begin{definition}[Predecessor]
    ...
    \begin{itemize}
      \item<+-> $P$ and $Q$ are normalized % Note: unique normal form
      \item<+-> $Q$ has not been found in any previous level % (otherwise it would be solvable in fewer comparisons)
      \item<+-> Adding a comparison $(i, j)$ to $Q$ results in $P$ % Note: subsequent normalization
      \item<+-> Adding $(j, i)$ results in a previously known poset % and subsequent normalization
    \end{itemize}
  \end{definition}
\end{frame}

% \begin{frame}{Predecessor Computation}
%   \item Annahme: alle sei in $\Omega_k$ die Menge aller in $k$ Schritten lösbare Posets sind 
%   \begin{definition}[Predecessor]
%     A normalized poset $Q$ is a \textbf{predecessor} of a set of known posets if there exists a comparison between two elements $u,v$ such that
%     \begin{enumerate}
%       \item Adding the relation $(u,v)$ to $Q$ yields a known poset $P_1$ solvable in $k_1$ steps
%       \item Adding the relation $(v,u)$ to $Q$ yields a known poset $P_2$ solvable in $k_2$ steps
%       \item The cost of $Q$ is $1 + \max(k_1, k_2)$
...
\begin{frame}{Example}
  ...
  \begin{figure}[!b]
    ...
    \caption{Search tree for $n=4,i=2,\text{upper\_bound}=4$}
...
\begin{frame}{Canonization}
  \begin{itemize}
    \item<+-> Canonization is essential
    \item<+-> nauty is expensive
    \item<+-> e.g., for $n=15,i=7$, nauty is used in only $0.042\%$ of cases
  \end{itemize}
\end{frame}
...
\begin{frame}{\insertsection}
  \begin{itemize}
    ...
    \item<+-> Correct some wrong lower bounds from Oksanen (e.g., for $n=15, i=4$) % he used a wrong heuristic
  ...
  \begin{table}[!t]
    ...
    \resizebox{1.0\textheight}{!}{%
      \begin{tabular}{c|c|l|l|l}
        ...
        14  & 2   & 0.0s                    & 1.5s                     & 0.0s             \\
        14  & 3   & 1.4s                    & 5.9s                     & 0.6s             \\
        14  & 4   & 35.9s                   & 46.9s                    & 1m 47s           \\
        14  & 5   & 17m 27s                 & 15m 33s                  & 6h 29m           \\
        14  & 6   & 2h 40m                  & 1h 40m                   & 4d 10h           \\
        14  & 7   & 14h 40m                 & 6h 27m                   & >5d              \\
        \hline
        15  & 1   & 0.0s                    & 0.0s                     & 0.0s             \\
        15  & 2   & 0.1s                    & 4.0s                     & 0.0s             \\
        15  & 3   & 2.8s                    & 25.9s                    & 1.4s             \\
        15  & 4   & 2m 24s                  & 13m 11s                  & 27m 17s          \\
        15  & 5   & 1h 12m                  & 45m 52s                  & 1d 5h 40m        \\
        15  & 6   & 1d 8h 37m               & 19h 30m                  & >5d              \\
        15  & 7   & 4d 23h 37m              & 1d 5h 43m                & >5d              \\
        15  & 8   & 14d 1h 51m              & 3d 8h 9m                 & >5d              \\
      \end{tabular}
    }
  \end{table}
\end{frame}
