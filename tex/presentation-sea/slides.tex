...
\begin{frame}{Complexity of the Selection Problem}
  ...
  \begin{itemize}
    ...
    \item<+-> The assumed lower bound for the median is $\approx 2.41n$ comparisons (Schönhage et al.).
  \end{itemize}
  \note{
    \begin{itemize}
...
\begin{frame}{Complexity of the Selection Problem}
  ...
  \note{
    \begin{itemize}
      ...
      \item And therefore elaborate how these exact values relate to the asymptotic bounds.
      \item This lays ground for elaborating the thightness of the bounds for the median.
      \item And also gives an idea where values for arbitrary $i$ are located in the spectrum.
...
\begin{frame}{Optimizations}
  ...
  \note{
    \begin{itemize}
      ...
      \item This hash value is computed for the root note and all other nodes iteratively, based on their in and out-degrees and the hash of the adjacent nodes.
...
\begin{frame}{Pruning techniques}
      ...
      child [dashed]{
        ...
        child [solid] {
            ...
            child [solid]{
                ...
                child[sibling distance = 1cm] {
                    node[] {$\{c,d\}$}
                    child[sibling distance = 1cm] { node[] {\color{red}not}}
                    child[sibling distance = 1cm] { node[] {\color{red}solved}}
                  }
                child[sibling distance = 1cm] { node[] {$\cdots$}}
              }
            child [solid]{
  ...
  \only<2>{
    ...
    \node[] {$(n,i,\emptyset)$}
      ...
      \draw[red] (-0.3,-1.5) -- (.4,-2);
      \draw[red] (-0.3,-2) -- (.3,-1.5);

      \draw[red] (1.7,-1.5) -- (2.4,-2);
      \draw[red] (1.7,-2) -- (2.4,-1.5);
...
% \sub{1}{44}
\begin{frame}<1-5>{\insertsection} % 1:44
...
\begin{frame}<1-4>{Predecessor Computation} % 0:50
...
\begin{frame}<1-3>{Removing Comparisons} % 0:44
  \begin{figure}[!b]
    \centering
...
\begin{frame}{Poset Distribution} % 0:28
  \begin{figure}[!b]
    \centering
...
\begin{frame}<1-5>{Contribution} % 1:01
...
\begin{frame}{Runtime Comparison} % 0:51
  \begin{table}[!t]
    \label{tab:times}
...
\begin{frame}<1-3>{Future Work} % 1:00
  \vspace{-1.2cm}
  \begin{columns}
...
\begin{frame}[shrink=25]{References} % 0:04
  \nocite{*}
